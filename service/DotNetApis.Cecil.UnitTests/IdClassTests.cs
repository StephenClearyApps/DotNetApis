...
namespace DotNetApis.Cecil.UnitTests
{
    public class IdClassTests
    {
        ...
        {
            var code = @"/// <summary>Text to find.</summary>
                        public class SampleClass<TFirst> { }";
            var (assembly, xmldoc) = Compile(code);
        ...
        {
            var code = @"/// <summary>Text to find.</summary>
                        public class SampleClass<TFirst, TSecond> { }";
            var (assembly, xmldoc) = Compile(code);
        ...
        {
            var code = @"public class OuterClass<TFirst, TSecond> {
                        /// <summary>Text to find.</summary>
                        public class SampleClass<TThird> { } }";
            var (assembly, xmldoc) = Compile(code);
        ...
        {
            var code = @"namespace Ns { public class OuterClass<TFirst, TSecond> {
                        /// <summary>Text to find.</summary>
                        public class SampleClass<TThird> { } } }";
            var (assembly, xmldoc) = Compile(code);
            var outer = assembly.Modules.SelectMany(x => x.Types).Single(x => x.Name == "OuterClass`2");
